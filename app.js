...
// ================================================================
//  2. REPORTS (Charts + Beautiful Occupancy Calendar)
// ================================================================
// ================================================================
//  REPORTS → Only Calendar (Calendly/Google Style)
// ================================================================
async function renderReports() {
  ...
  const [rooms, bookings] = await Promise.all([
    ...
    sb.from('guest_register').select('room_id, check_in, check_out, guest_name, booking_mode'),
  ]);
  ...
  // Build booking map: room_id_date → guest name & mode
  const bookingMap = {};
  (bookings.data||[]).forEach(b => {
    ...
    while (d < end) {
      const key = `${b.room_id}_${d.toISOString().slice(0,10)}`;
      bookingMap[key] = { guest: b.guest_name, mode: b.booking_mode };
      d.setDate(d.getDate()+1);
    }
  });

  // Build HTML - Google Calendar style
  let html = `
  ...
  (rooms.data||[]).forEach(room => {
    ...
    for (let d=1; d<=daysInMonth; d++) {
      ...
      if (booking) {
        ...
        label = `<span style="font-size:11px;display:block;margin-top:-2px;">${guestName}</span>`;
      }
  ...
  // Store month state for navigation
