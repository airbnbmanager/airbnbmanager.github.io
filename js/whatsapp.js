...
function showInvestorAlertModal(propertyName, investors, msg, groupLink, groupName) {
  ...
  modal.innerHTML = `
    <div class="modal-box" style="max-width:600px;">
      ...
      <div class="btn-row" style="margin-top:12px;flex-wrap:wrap;">
        ${groupLink ? `<button style="background:#128C7E;color:#fff;font-weight:700;" onclick="
          navigator.clipboard.writeText(document.getElementById('waMsg').value);
          window.open('${groupLink}', '_blank');
          setTimeout(function(){ if(window.fsn) fsn.success('✅ Message Copied', 'Group khul gaya — paste karo message'); }, 500);
        ">📢 Send to Group (${groupName || 'Property Group'})</button>` : '<button disabled style="background:#ccc;color:#666;cursor:not-allowed;" title="Add WhatsApp group link in Property Edit">📢 No Group Link (add in Property Edit)</button>'}
        ${recipients}
        <button class="outline" onclick="navigator.clipboard.writeText(document.getElementById('waMsg').value);fsn.success('Copied','Message copied')">
          📋 Copy
        </button>
