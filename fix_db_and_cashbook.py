import urllib.request, json, glob, re

sb_url, sb_key = None, None
for fpath in glob.glob('js/*.js') + ['index.html', 'admin.html']:
    try:
        with open(fpath, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
            m_url = re.search(r'https://[a-z0-9]+\.supabase\.co', content)
            m_key = re.search(r'eyJ[a-zA-Z0-9_\-\.]{50,}', content)
            if m_url and not sb_url: sb_url = m_url.group(0)
            if m_key and not sb_key: sb_key = m_key.group(0)
    except: pass
...
if sb_url and sb_key:
    ...
    # Fetch handovers
    req = urllib.request.Request(f'{sb_url}/rest/v1/cash_handovers?select=*', headers=headers)
    with urllib.request.urlopen(req) as r:
        handovers = json.loads(r.read().decode())

    # 1. Clean bad Aniket handovers from DB
    for h in handovers:
        if (h.get('from_person') or '').lower() == 'aniket':
            del_req = urllib.request.Request(f"{sb_url}/rest/v1/cash_handovers?id=eq.{h['id']}", headers=headers, method='DELETE')
            urllib.request.urlopen(del_req)
            print("Cleaned invalid Aniket handover ID:", h['id'])

    # 2. Ensure Shahenshah -> Firoz (₹6,000) exists in DB
    has_shahenshah = any((h.get('from_person') or '').lower() == 'shahenshah' and (h.get('to_person') or '').lower() == 'firoz' for h in handovers)
    if not has_shahenshah:
        data = json.dumps({'from_person': 'Shahenshah', 'to_person': 'Firoz', 'amount': 6000, 'handover_date': '2026-08-24', 'notes': 'Cash Handover to Firoz'}).encode()
        req = urllib.request.Request(f'{sb_url}/rest/v1/cash_handovers', data=data, headers=headers, method='POST')
        urllib.request.urlopen(req)
        print("Added Shahenshah -> Firoz 6,000 to DB")

    # 3. Ensure Hazi Sahab -> Firoz (₹16,000) exists in DB
    has_hazi = any('hazi' in (h.get('from_person') or '').lower() and (h.get('to_person') or '').lower() == 'firoz' for h in handovers)
    if not has_hazi:
        data = json.dumps({'from_person': 'Mr. Alam Hazi Sahab', 'to_person': 'Firoz', 'amount': 16000, 'handover_date': '2026-08-24', 'notes': 'Cash Handover to Firoz'}).encode()
        req = urllib.request.Request(f'{sb_url}/rest/v1/cash_handovers', data=data, headers=headers, method='POST')
        urllib.request.urlopen(req)
        print("Added Mr. Alam Hazi Sahab -> Firoz 16,000 to DB")
    ...
    <div style="border:1px solid #E5E7EB;border-radius:10px;padding:14px;margin-bottom:10px;background:#fff;">
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:12px;">
        <div style="padding:8px;background:#DCFCE7;border-radius:6px;text-align:center;">
          <div style="font-size:10px;color:#166534;font-weight:600;">Received (All Time)</div>
